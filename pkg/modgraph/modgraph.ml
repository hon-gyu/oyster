...
let collect_mod_refs ~iterator ~refs mod_expr =
  ...
  | _ ->
    iterator.Tast_iterator.module_expr iterator mod_expr
  ...
  | Cmt_format.Implementation str ->
    ...
    List.iter submod_refs ~f:(fun (submod, refs) ->
      ...
      Hash_set.iter refs ~f:(fun ref_name ->
        ...
        let dst =
          if String.is_prefix ref_name ~prefix:(unit_name ^ ".") then ref_name
          else if String.mem ref_name '.' then ref_name
