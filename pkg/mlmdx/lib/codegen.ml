...
let node ?(attrs = []) tag ~children =
  jsx "node" [ estring tag; elist attrs; elist children ]
;;

(* Markdown-structural elements route through the overridable components table
   (the [components] parameter of the generated [make]) rather than a hardcoded
...
let md_elt tag ~children =
  Ast_helper.Exp.apply
    (Ast_helper.Exp.field (Ast_helper.Exp.ident (lident "components")) (lident tag))
    [ Asttypes.Labelled "children", elist children ]
;;
...
let parse_expr ~file ~lnum ~bol ~cnum src =
  ...
  Lexing.set_position
    lb
    { Lexing.pos_fname = file; pos_lnum = lnum; pos_bol = bol; pos_cnum = cnum };
  Parse.expression lb
;;
...
let brace_close s i =
  ...
  let rec code k depth =
    if k >= n
    then failwith "mlmdx: unterminated { in JSX attribute"
    else (
      match s.[k] with
      | '{' -> code (k + 1) (depth + 1)
  ...
  and string quote k depth =
    if k >= n
    then failwith "mlmdx: unterminated string in JSX attribute"
    else (
      match s.[k] with
  ...
  and comment k depth nest =
    if k >= n
    then failwith "mlmdx: unterminated comment in JSX attribute"
...
let quote_close s quote i =
  ...
  let rec go k =
    if k >= n
    then failwith "mlmdx: unterminated quoted JSX attribute"
    else (
      match s.[k] with
      | '\\' when k + 1 < n -> go (k + 2)
      | c when c = quote -> k
...
let parse_raw_tag ~origin ~file ~meta raw =
  ...
  let expr_span a b =
    parse_expr ~file ~lnum ~bol ~cnum:(base + a) (String.sub raw a (b - a))
  in
  let n = String.length raw in
  if n < 2 || raw.[0] <> '<' then failwith "mlmdx: invalid JSX opening tag";
  if n >= 2 && raw.[1] = '>'
  then { name = ""; attrs = []; self_closing = false }
  else (
    let i = ref 1 in
    let name_start = !i in
    while !i < n && not (is_name_stop raw.[!i]) do
      incr i
    done;
          ...
          let value =
            ...
            then (
              ...
              | ('"' | '\'') as quote ->
                ...
                i := close + 1;
                String_value value
              | _ -> failwith "mlmdx: JSX attribute values must be quoted or braced")
            else Absent
          in
...
let component_call tag attrs ?children () =
  ...
  let args =
    match children with
    | None -> args
    | Some e -> args @ [ Asttypes.Labelled "children", e ]
  in
...
let rec inline ~origin ~file (i : Inline.t) : Parsetree.expression =
  ...
  | Inline.Emphasis (e, _) ->
    md_elt "em" ~children:[ inline ~origin ~file (Inline.Emphasis.inline e) ]
  | Inline.Strong_emphasis (e, _) ->
    md_elt "strong" ~children:[ inline ~origin ~file (Inline.Emphasis.inline e) ]
  | Inline.Code_span (cs, _) ->
    md_elt "code" ~children:[ jstring (Inline.Code_span.code cs) ]
  | Inline.Inlines (is, _) -> jlist (List.map (inline ~origin ~file) is)
  | Inline.Break (_, _) -> jstring " "
  | Inline.Ext_jsx_expr (j, m) ->
    embedded_expr ~origin ~file ~meta:m (Inline.Jsx_expr.expr j)
  | Inline.Ext_jsx_element (e, m) -> jsx_inline_element ~origin ~file ~meta:m e
  (* TODO: unhandled inline constructs (links, images, autolinks, raw inline
     HTML) silently drop to [jnull]. Broaden coverage (links/images first, which
     add the first prop-bearing [components] fields), and make this catch-all
     loud rather than silent for anything still unsupported. *)
  | _ -> jnull
...
and jsx_inline_element ~origin ~file ~meta e =
  ...
  | name, None -> node name ~attrs:(List.map host_attr tag.attrs) ~children:[]
  | name, Some child ->
...
and block ~origin ~file (b : Block.t) : Parsetree.expression =
  ...
  | Block.Heading (h, _) ->
    ...
    md_elt tag ~children:[ inline ~origin ~file (Block.Heading.inline h) ]
  | Block.Paragraph (p, _) ->
    md_elt "p" ~children:[ inline ~origin ~file (Block.Paragraph.inline p) ]
  | Block.Blocks (bs, _) -> jlist (List.map (block ~origin ~file) bs)
  | Block.Block_quote (bq, _) ->
    md_elt "blockquote" ~children:[ block ~origin ~file (Block.Block_quote.block bq) ]
  | Block.Ext_jsx_block (j, _) -> jsx_block ~origin ~file j
  (* TODO: unhandled block constructs (lists, fenced/indented code, thematic
     breaks, HTML blocks) silently drop to [jnull]. Broaden coverage (lists
     first, reusing the block recursion; fenced code carries a language for
     later highlighting), and make this catch-all loud for the rest. *)
  | _ -> jnull
...
let structure ~origin ~file (doc : Doc.t) : Parsetree.structure =
  ...
  (* ?(components : Mlmdx.Components.t = Mlmdx.Components.default): the annotation
     lets the [components.h1] field accesses in [body] disambiguate without an
     [open], and the default makes the table a no-op unless a caller overrides. *)
  let components_param : Parsetree.function_param =
    let pat =
      Ast_helper.Pat.constraint_
        (Ast_helper.Pat.var (mknoloc "components"))
        (Ast_helper.Typ.constr (mknoloc (components_lident "t")) [])
    in
    { pparam_loc = Location.none
    ; pparam_desc =
        Pparam_val
          ( Asttypes.Optional "components"
          , Some (Ast_helper.Exp.ident (mknoloc (components_lident "default")))
          , pat )
    }
  in
  let unit_param : Parsetree.function_param =
    { pparam_loc = Location.none
    ; pparam_desc =
        Pparam_val (Asttypes.Nolabel, None, Ast_helper.Pat.construct (lident "()") None)
    }
  in
  let make =
    Ast_helper.Exp.function_ [ components_param; unit_param ] None (Pfunction_body body)
  in
...
let line_starts_with s first last prefix =
  ...
  last - first >= plen
  && String.sub s first plen = prefix
  &&
...
let split_initial_prelude s =
  ...
  let rec consume i consumed_any =
    ...
    then n, n
      ...
      if prelude_starter s first first_end
      then (
        ...
        then
          failwith
            "mlmdx: top-of-file OCaml prelude must be separated from Markdown by a blank \
             line";
        consume after true)
...
let of_string ~file s : Parsetree.structure =
  ...
  let prelude =
    if split.prelude = ""
    then []
    else parse_structure ~file ~origin:source_start split.prelude
  in
  let doc =
    Doc.of_string
      ~strict:false
...
let%expect_test "codegen: heading with embedded expr" =
  print_string
    (Pprintast.string_of_structure (of_string ~file:"t.mlmdx" "# {JSX.int (2 + 2)}"));
  [%expect
    {|
    let make ?components:((components : Mlmdx.Components.t)=
      Mlmdx.Components.default) () = components.h1 ~children:[JSX.int (2 + 2)]
    |}]
;;
...
let%expect_test "codegen: paragraph with prose, bold, and embedded expr" =
  ...
  [%expect
    {|
    let make ?components:((components : Mlmdx.Components.t)=
      Mlmdx.Components.default) () =
      components.p
        ~children:[JSX.list
                     [JSX.string "Some ";
                     components.strong ~children:[JSX.string "bold"];
                     JSX.string " and ";
                     JSX.string name;
                     JSX.string "."]]
    |}]
...
let%expect_test "codegen: self-closing component with attributes" =
  ...
  [%expect
    ...
    let make ?components:((components : Mlmdx.Components.t)=
      Mlmdx.Components.default) () =
      components.p
        ~children:[Greeting.make ~name:"World" ~count:(1 + 2) ~loud:true ()]
    |}]
;;
...
let%expect_test "codegen: component inline JSX children" =
  ...
  [%expect
    ...
    let make ?components:((components : Mlmdx.Components.t)=
      Mlmdx.Components.default) () =
      components.p
        ~children:[Callout.make ~kind:"warn"
                     ~children:(JSX.list
                                  [JSX.string "Read ";
                                  components.strong ~children:[JSX.string "this"]])
                     ()]
    |}]
;;
    ...
    let make ?components:((components : Mlmdx.Components.t)=
      ...
      JSX.list
        [JSX.node "div" [("class", (`String "box"))]
           [JSX.list
              [JSX.null; components.h1 ~children:[JSX.string "Title"]; JSX.null]];
        JSX.null;
        JSX.list
          [JSX.list
             [JSX.null; components.p ~children:[JSX.string "text"]; JSX.null]]]
    |}]
;;
...
Body
  ...
  [%expect
    ...
    let make ?components:((components : Mlmdx.Components.t)=
      Mlmdx.Components.default) () =
      JSX.list
        [components.h1 ~children:[JSX.string title];
        JSX.null;
        Panel.make ~title
          ~children:(JSX.list
                       [JSX.null;
                       components.p ~children:[JSX.string "Body"];
                       JSX.null]) ()]
    |}]
;;
  ...
  [%expect
    ...
    let make ?components:((components : Mlmdx.Components.t)=
      ...
      JSX.list
        [components.h1 ~children:[JSX.string "Title"];
        JSX.null;
        components.p ~children:[JSX.string "let x = 1"]]
    |}]
;;

let%expect_test "codegen: split_initial_prelude" =
  let split =
    split_initial_prelude
      {|open Components
...
|}
  ...
  Printf.printf
    ...
    split.prelude
    split.body
    split.body_origin.line
    split.body_origin.byte;
  [%expect
    {|
