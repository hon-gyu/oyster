...
let frag_to_string = function
  | None -> "None"
  | Some (Wikilink.Heading hs) ->
    sprintf "Heading [%s]" (String.concat "; " hs)
  | Some (Wikilink.Block_ref s) -> sprintf "Block_ref %s" s

let opt_to_string = function None -> "None" | Some s -> s
...
let test_parse name ~embed input expected_target expected_fragment expected_display =
  ...
  if w.target <> expected_target then begin
    printf "FAIL [%s]: target = %s, expected %s\n" name
      (opt_to_string w.target) (opt_to_string expected_target);
    pass := false
  end;
  if not (frag_equal w.fragment expected_fragment) then begin
    printf "FAIL [%s]: fragment = %s, expected %s\n" name
      (frag_to_string w.fragment) (frag_to_string expected_fragment);
    pass := false
  end;
  if w.display <> expected_display then begin
    printf "FAIL [%s]: display = %s, expected %s\n" name
      (opt_to_string w.display) (opt_to_string expected_display);
    pass := false
  end;
  if w.embed <> embed then begin
...
let test_parse_content () =
  test_parse "basic note" ~embed:false "Note"
    (Some "Note") None None;
  test_parse "note with ext" ~embed:false "Note.md"
    (Some "Note.md") None None;
  test_parse "dir path" ~embed:false "dir/Note"
    (Some "dir/Note") None None;
  test_parse "display text" ~embed:false "Note|custom text"
    (Some "Note") None (Some "custom text");
  test_parse "heading" ~embed:false "Note#Heading"
    (Some "Note") (Some (Wikilink.Heading ["Heading"])) None;
  test_parse "nested heading" ~embed:false "Note#H1#H2"
    (Some "Note") (Some (Wikilink.Heading ["H1"; "H2"])) None;
  test_parse "current note heading" ~embed:false "#Heading"
    None (Some (Wikilink.Heading ["Heading"])) None;
  test_parse "block ref" ~embed:false "Note#^blockid"
    (Some "Note") (Some (Wikilink.Block_ref "blockid")) None;
  test_parse "block ref with hyphen" ~embed:false "Note#^block-id"
    (Some "Note") (Some (Wikilink.Block_ref "block-id")) None;
  test_parse "invalid block id (underscore)" ~embed:false "Note#^block_id"
    (Some "Note") (Some (Wikilink.Heading ["^block_id"])) None;
  test_parse "block ref current note" ~embed:false "#^blockid"
    None (Some (Wikilink.Block_ref "blockid")) None;
  test_parse "heading with display" ~embed:false "#H1#H2|text"
    None (Some (Wikilink.Heading ["H1"; "H2"])) (Some "text");
  test_parse "embed" ~embed:true "Note"
    (Some "Note") None None;
  test_parse "hash collapse" ~embed:false "##A###B"
    None (Some (Wikilink.Heading ["A"; "B"])) None;
  test_parse "heading then block ref literal" ~embed:false "Note#H1#^blockid"
    (Some "Note") (Some (Wikilink.Heading ["H1"; "^blockid"])) None;
  test_parse "empty target with hash" ~embed:false "#"
    None None None;
  test_parse "empty target" ~embed:false ""
    None None None
...
let test_scan () =
  ...
  if Option.is_some r then printf "FAIL [scan no wikilinks]: expected None\n"
  ...
  (match Wikilink.scan "before [[Note]] after" meta with
   ...
   | Some inlines ->
     ...
     if n = 1 then printf "OK   [scan single]\n"
  ...
  (match Wikilink.scan "[[A]] and [[B]]" meta with
   ...
   | Some inlines ->
     let n = count_wikilinks inlines in
     if n = 2 then printf "OK   [scan multiple]\n"
...
let test_block_id () =
  let test name input expected =
    ...
    let pass =
      ...
      | None, None -> true
      | Some (before, id), Some (exp_before, exp_id) ->
        before = exp_before && id = exp_id
      | _ -> false
    in
    if pass then printf "OK   [block_id %s]\n" name
    else begin
      let show = function
        | None -> "None"
        | Some (b, i) -> sprintf "(%s, %s)" b i
      in
      printf "FAIL [block_id %s]: got %s, expected %s\n" name
...
let rec collect_inline_from_block = function
  ...
  | Cmarkit.Block.Blocks (bs, _) ->
    List.concat_map collect_inline_from_block bs
  | _ -> []
...
let test_integration () =
  ...
  if found then printf "OK   [integration wikilink]\n"
  ...
  (match block with
   | Cmarkit.Block.Paragraph (_, meta) ->
     (match Cmarkit.Meta.find Block_id.meta_key meta with
      | Some id ->
        if id = "myblock" then printf "OK   [integration block_id]\n"
