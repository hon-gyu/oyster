...
(** Replace the embed paragraphs of [doc], the note at [curr_path], with
    ...
    @param max_depth Inclusive depth limit. When [embed_depth >= max_depth],
      ...
      {!Note.Transclusion.fallback_block} for a wikilink, the original paragraph
      for an image.
...
let rec expand_doc
  ...
  let embed (link_ref : Note.Link.Ref.t) ~(depth_fallback : Cmarkit.Block.t)
    ...
    | Ok target ->
      ...
      (* A self-reference uses [doc] unexpanded; expanding it would not
         terminate. *)
      let source =
      ...
      Option.map source ~f:(fun source ->
        Note.Transclusion.transclude
          ~depth
          ~source_path
          ~fragment
          (select (Note.Transclusion.non_fm_blocks source)))
  in
  let mapper =
    Cmarkit.Mapper.make
      ...
      ~block:(fun _mapper block ->
        ...
        | Cmarkit.Block.Paragraph (p, _) ->
          (match
             Note.Transclusion.embed_source_of_inline (Cmarkit.Block.Paragraph.inline p)
           with
