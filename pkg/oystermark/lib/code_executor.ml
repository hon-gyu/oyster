...
let run_notebook ~(uv_config : uv_config) ~(nb_json : Yojson.Basic.t)
  ...
  let cmd =
    sprintf
      "uv run --python %g %s jupyter nbconvert --to notebook --execute %s --output %s \
       2>/dev/null"
      uv_config.version
      with_args
      tmp_in
...
let cell_outputs (cell : Yojson.Basic.t) : string list =
  ...
  |> List.filter_map ~f:(fun output ->
    ...
    | "stream" -> Some (output |> member "text" |> to_string)
...
let uv_executor ?(attr_filter : Attribute.t option -> bool = fun _ -> true) : executor =
  ...
  let (python_cells : cell list) =
    List.filter ctx.inputs ~f:(fun cell ->
      ...
      | None -> false)
  in
  let (sources : string list list) =
    List.map python_cells ~f:(fun cell -> String.split_lines cell.content)
  in
