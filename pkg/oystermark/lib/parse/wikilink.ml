...
let gen_safe_string : string Quickcheck.Generator.t =
  ...
  |> filter ~f:(fun s ->
       not (String.is_substring s ~substring:"[[" || String.is_substring s ~substring:"]]"))
;;
...
let gen_block_id : string Quickcheck.Generator.t =
  ...
  let body_char =
    Quickcheck.Generator.union [ Char.gen_alphanum; return '-' ]
  in
  Let_syntax.(
...
let%test_unit "roundtrip: make (content_of (to_commonmark wl)) = wl" =
  Quickcheck.test quickcheck_generator ~sexp_of:sexp_of_t ~f:(fun wl ->
      ...
      Error.raise_s
        [%message
          "roundtrip failed"
            ~expect:(wl : t)
            ~got:(parsed : t)
            ~commonmark:(to_commonmark wl : string)])
