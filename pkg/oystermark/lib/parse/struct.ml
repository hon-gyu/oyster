...
module For_test = struct
  ...
  let inline_value_list_item =
    mk_example "inline_value_list_item" {|- foo: bar|}
  ;;
  ...
  let no_body_no_following =
    mk_example "no_body_no_following" {|- foo:|}
  ;;

  (* Chains — spec §5-8 *)

  (** spec §5: [- a: b: c] — chain of 2 labels + inline value *)
  let chain_with_value =
    mk_example "chain_with_value" {|- a: b: c|}
  ;;
  ...
  let two_independent_siblings =
    mk_example "two_independent_siblings" {|- a: b: c
- x: y|}
  ;;
  ...
  let paragraph_inline_value =
    mk_example "paragraph_inline_value" {|foo: bar|}
  ;;
...
- qux|}
  ...
  let mixed_inline_value =
    mk_example "mixed_inline_value" {|- foo: bar *baz* qux|}
  ;;

  (** spec §12: code span in value — only [foo] is the label, code interior is opaque *)
  let code_span_in_value =
    mk_example "code_span_in_value" {|- foo: `code: thing`|}
  ;;
  ...
  let url_as_label =
    mk_example "url_as_label" {|- http://x.com: click here|}
  ;;
...
- bar|}
  ...
  let emphasis_inline_value =
    mk_example "emphasis_inline_value" {|- *foo*: bar|}
  ;;
...
following|}
  ...
  let free_form_value =
    mk_example "free_form_value" {|- foo: *bar* x|}
  ;;
