...
let lucide_icon_body (name : string) : string option =
  ...
  | "lucide-check" ->
    Some "<path d='M20 6 9 17l-5-5'/>"
  | "lucide-circle-help" ->
    Some
      "<circle cx='12' cy='12' r='10'/><path d='M9.09 9a3 3 0 0 1 5.83 1c0 2-3 \
       3-3 3'/><path d='M12 17h.01'/>"
  | "lucide-triangle-alert" ->
    Some
      "<path d='m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 \
       1.73-3'/><path d='M12 9v4'/><path d='M12 17h.01'/>"
  | "lucide-x" ->
    Some "<path d='M18 6 6 18'/><path d='m6 6 12 12'/>"
  | "lucide-zap" ->
    Some
  ...
  | "lucide-bug" ->
    Some
      "<path d='m8 2 1.88 1.88'/><path d='M14.12 3.88 16 2'/><path d='M9 7.13v-1a3.003 \
       3.003 0 1 1 6 0v1'/><path d='M12 20c-3.3 0-6-2.7-6-6v-3a4 4 0 0 1 4-4h4a4 4 0 0 \
       1 4 4v3c0 3.3-2.7 6-6 6'/><path d='M12 20v-9'/><path d='M6.53 9C4.6 8.8 3 7.1 3 \
  ...
  | "lucide-quote" ->
    Some "<path d='M6 21 3 9h4l3-6'/><path d='M17 21 14 9h4l3-6'/>"
  | "lucide-bot-message-square" ->
    Some
      "<path d='M12 6V2H8'/><path d='m8 18-4 4V8a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 \
       0 0 1-2 2Z'/><path d='M2 12h2'/><path d='M9 11v2'/><path d='M15 11v2'/><path \
       d='M20 12h2'/>"
...
let expand (css : string) : string =
  ...
  let rec scan (pos : int) =
    ...
    then ()
      ...
      | None ->
        Buffer.add_string buf (String.sub css ~pos ~len:(len - pos))
      | Some brace_open ->
         ...
         | Some brace_close ->
           ...
           let body =
             String.sub
               css
               ~pos:(brace_open + 1)
               ~len:(brace_close - brace_open - 1)
           in
           let block = parse_block selector_text body in
           let has_callout_icon =
             List.exists block.props ~f:(fun (p, _) ->
               String.equal p "--callout-icon")
           in
           if has_callout_icon
           then (
             let icon_name =
             ...
             Buffer.add_string
               buf
               (String.sub css ~pos ~len:(brace_close + 1 - pos));
             scan (brace_close + 1))))
  in
  scan 0;
  Buffer.contents buf
