...
> This is an error message"#;
        ...
        // The BlockQuote should have been transformed into a Callout
        if let NodeKind::Callout {
            kind,
            title,
            foldable,
            content_start_byte,
        ...
        {
            assert_eq!(*kind, CalloutKind::Obsidian(ObsidianCalloutKind::Danger));
            assert_eq!(*title, None);
            assert_eq!(*foldable, None);
            assert!(*content_start_byte > 0);
        } else {
            panic!("Expected Callout node, got {:?}", callout_node.kind);
        }
    }
...
> Like this one."#;
        ...
        {
            assert_eq!(*kind, CalloutKind::Obsidian(ObsidianCalloutKind::Info));
            assert_eq!(
                *title,
...
> This is a standard note"#;
        ...
        // Should be recognized as standard GFM callout
        if let NodeKind::Callout {
            kind,
            content_start_byte,
            ..
